...
restic -r ${RESTIC_ROOT_DIR}/backup/repository forget --keep-daily ${RESTIC_BACKUP_KEEP_DAILY} --keep-weekly ${RESTIC_BACKUP_KEEP_WEEKLY} --keep-monthly ${RESTIC_BACKUP_KEEP_MONTHLY} --keep-yearly ${RESTIC_BACKUP_KEEP_YEARLY} --group-by paths --prune
